...
def get_olympiad_file_content(name, category, round, dates_sk, webpage, min_age, max_age, info=""):
    name, category, round, info = map(lambda s: s.strip().replace("-", "–"), (name, category, round, info))
    return f"""# Autogenerated from "{INPUT_FILE_NAME}" using "/utils/create_olympiads_from_table.py"
...
contestants:
  min: {min_age}
  max: {max_age}{f'''
info: {info.replace('"', '\\"')}''' if len(info) > 0 else ""}
"""

def generate_olympiad_files(olympiad: list, reset=True):
    assert len(olympiad) > 1

    if len(olympiad[0].split("\t")) == 1: name, webpage = olympiad.pop(0), ""
    else: name, webpage = olympiad.pop(0).split("\t")
    if len(webpage) == 0: webpage = "https://nivam.sk/"
    if not "https://" in webpage: webpage = "https://" + webpage

    print(f"{name}...", end="")

    info = olympiad.pop(0)[1:].strip() if olympiad[0].strip()[0] == ':' else ""

    dir_name = "./" + name.replace(" ", "_").lower()
    if (reset and os.path.isdir(dir_name)): shutil.rmtree(dir_name)
    if not os.path.isdir(dir_name): os.mkdir(dir_name)

    for line in map(lambda l: l.split("\t"), olympiad):
        ...
        for round, dates_sk in filter(lambda rd: len(rd[1].strip()) > 0, zip(
            ...
            text = get_olympiad_file_content(name, category, round, dates_sk, webpage, min_age, max_age, info)
